...
int velocity = 50; //chord volume
int melVelocity = 100; //melody note volume
int ticks = noteLen; //length in milliseconds
int nbeats = 4; //Beats per measure
int trackBeat = 1;

//General logistics stuff
int fudgetime = 0; //Delay between computer and xylobot (computer plays first); set to 0 if no computer
...
ArrayList<NoteMessage> chordNotes = new ArrayList();
ArrayList<NoteMessage> melodyNotes = new ArrayList();
...
Measure generateMeasure(int numTonic){
   ...
   for(int x = 0; x < out.nbeats; x++){
     if (!(tonicTotal == -1 || tonicCount < numTonic)){
       ...
       switch((nbeats-x-1)%3){
          case 0:
            tempBeat.notes[0].add(new NoteMessage(tonic, melVelocity, channel));
            break;
          case 1:
            tempBeat.notes[0].add(new NoteMessage(tonic+7, melVelocity, channel));
            break;
          case 2:
            tempBeat.notes[0].add(new NoteMessage(tonic+4, melVelocity, channel)); //If minor, change this to +3
            break;
       }
 ...
 Beat generateBeat(int c){
    ...
    ArrayList<NoteMessage>[] onotes = new ArrayList[nsubbeats];
    for(int x = 0; x < nsubbeats; x++){
       onotes[x] = new ArrayList<NoteMessage>(); 
    }
    ArrayList<NoteMessage>[] oenotes = new ArrayList[nsubbeats];
    for(int x = 0; x < nsubbeats; x++){
       oenotes[x] = new ArrayList<NoteMessage>(); 
    }
 ...
 ArrayList<NoteMessage> generateChord(int c, ArrayList<NoteMessage>[] oenotes){
   ArrayList<NoteMessage> output = new ArrayList<NoteMessage>();
    ...
    for (i = 0; i < chords[c].length; i++) {
      ...
      if (randomNum < sum) {
        ...
        for (int x = 0; x < notes.length; x++) {
          oenotes[0].add(new NoteMessage(notes[x], velocity, channel));
 ...
 boolean generateMelody(int subbeatlen, ArrayList<NoteMessage>[] onotes, ArrayList<String>[] onotetext){
  ...
  for(int i = 0; i < nsubbeats; i++){
    if (i % (nsubbeats/divisions[randNum])==0) {
      ...
      NoteMessage melody = new NoteMessage(melnote, melVelocity, channel);
      onotes[i].add(melody);
    ...
    if (fuzzyContains(beatIndex + 1 + (float)(i)/nsubbeats, snarebeats, thresh)) {
      NoteMessage snareNote = new  NoteMessage(36, melVelocity, pchannel1);
    ...
    if (fuzzyContains(beatIndex + 1 + (float)(i)/nsubbeats, bassbeats, thresh)) {
      NoteMessage bassNote = new  NoteMessage(38, melVelocity, pchannel2);
