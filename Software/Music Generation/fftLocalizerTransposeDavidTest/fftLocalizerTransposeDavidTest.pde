...
void draw()
{
  ...
  if (hearNotes){
    //Need to normalize beatProbsArr so they all together sum to 1, else comparisons between notes are silly
    beatProbSum = 0;
    for (int h = 0; h < beatProbsArr.length; h++)
    {
      for (int i = 0; i < (bucketsPerRhythm+1); i++) {
        beatProbSum += beatProbsArr[h].get(i, 0); //TODO multiply by onemat instead
      }
    }
    for (int h = 0; h < beatProbsArr.length; h++)
    {
      for (int i = 0; i < (bucketsPerRhythm+1); i++) {
        beatProbsArr[h].set(i, 0, beatProbsArr[h].get(i, 0) / beatProbSum); //TODO timesequals instead
      }
    }
  }
  //for (int i = 0; i < beatProbsArr.length; i++)
  ...
  boolean heardBeat = (hearNotes && amp.analyze() > beatThreshScale * beatThresh && (ignorePitch || hasPitch));
        ...
        if (isBeat) {
          ...
          else
          {
            if (heardBeat) { //We know we're not ignoring pitch because DeMorgan stuff
              ...
              tempil *= beatProbs.get(i, 0); //Still want to multiply in probability of something being there - it's more likely you play the wrong note at the right time than the wrong note at the wrong time
